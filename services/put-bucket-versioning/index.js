...
server.addService(svc.PutBucketVersioning.service,
  {
    "PutBucketVersioning": putBucketVersioning
  }
);

async function putBucketVersioning({ request }, cb) {
  const { bucketName, versioningStatus } = request;

  try {
    const statusCode = await checkAuth(bucketName, "", "B", "put", requesterUName);
    if (statusCode !== 200) return cb(null, { statusCode: statusCode })
  
    await bucket.putBucketVersioning(bucketName, versioningStatus);
