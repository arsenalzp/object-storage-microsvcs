...
func init() {
	...
	_, err := DbConnect()
	if err != nil {
		log.Printf("db client init error:%+v\n", errors.New("db client error", errors.DbClntInitErr, err))
	}
}
...
func DbConnect() (*mongo.Database, error) {
	...
	if db != nil {
		...
		if err := client.Ping(context.TODO(), readpref.Primary()); err != nil {
			err = errors.New("db client error", errors.DbConnErr, err)
			return nil, err
		}
		return db, nil
	}
	...
	if err := client.Ping(context.TODO(), readpref.Primary()); err != nil {
		err = errors.New("db client error", errors.DbClntErr, err)
		return nil, err
	}
